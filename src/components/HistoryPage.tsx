...
export default function HistoryPage({ articles, onBack }: HistoryPageProps) {
  ...
  const stats = {
    ...
    avgAccuracy:
      completedArticles.length > 0
        ...
        : 0,
    totalTime: completedArticles.reduce((sum, a) => sum + a.readingTime, 0),
    streak: 12,
  };

  // Filter articles
  const filteredArticles = completedArticles.filter((article) => {
    if (filter === "high") return (article.accuracy || 0) >= 80;
    if (filter === "week") {
  ...
  // Mock heatmap data (last 12 weeks)
    ...
    <div className="min-h-screen bg-white">
      ...
      <div className="max-w-6xl mx-auto px-6 py-12">
        ...
        <div className="bg-white rounded-xl p-8 shadow-sm border border-app-gray-200 mb-12">
          <div className="flex items-center justify-between mb-8">
            ...
            <div className="flex gap-2">
              {["7일", "30일", "3개월"].map((period) => (
                <button
                  key={period}
                  className="px-4 py-1.5 rounded-full text-sm bg-app-gray-50 hover:bg-app-gray-200 text-app-gray-500 transition-colors"
                >
                  {period}
                </button>
              ))}
          ...
          <div className="h-64 flex items-end justify-between gap-4">
            {chartData.map((value, idx) => (
              ...
              >
                <div
                  className="w-full bg-linear-to-t from-app-blue to-blue-400 rounded-t-lg transition-all hover:opacity-80 cursor-pointer relative group"
        ...
        <div className="bg-white rounded-xl p-8 shadow-sm border border-app-gray-200 mb-12">
          ...
          <div className="flex items-center justify-end gap-2 mt-4 text-xs text-app-gray-500">
            <span>Less</span>
            {[0, 1, 2, 3, 4].map((level) => (
              <div
                key={level}
                ...
                <div className="flex items-center justify-between">
                  <div className="flex-1 pr-6">
                    ...
                    <div className="flex items-center gap-3 text-sm text-app-gray-500 mb-3">
                      ...
                      <span>•</span>
                      <span>
                        {Math.floor(article.readingTime / 60)}분{" "}
                        {article.readingTime % 60}초
                      </span>
                    </div>
                    <div className="flex gap-2">
                  ...
                  <div className="flex flex-col items-center gap-3">
                    <div
                      className={`w-16 h-16 rounded-full bg-linear-to-br ${getAccuracyColor(
                        article.accuracy
                      )} flex items-center justify-center shadow-md`}
                    >
                      <span className="text-white">{article.accuracy}%</span>
                    </div>
                    <button className="text-sm text-app-blue hover:underline">
                      보기 →
                    </button>
                  </div>
                </div>
