    ...
    return (
      <AlertDialogPortal>
        ...
        <div
          ...
          data-state={open ? 'open' : 'closed'}
          className={cn(
            'bg-background data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95 fixed top-[50%] left-[50%] z-50 grid w-full max-w-[calc(100%-2rem)] translate-x-[-50%] translate-y-[-50%] gap-4 rounded-lg border border-app-gray-200 p-6 shadow-lg duration-200 sm:max-w-lg',
            className,
          )}
          {...props}
        >
...
function AlertDialogTitle({ className, ...props }: AlertDialogTitleProps) {
  return (
    <h2 data-slot="alert-dialog-title" className={cn('text-lg leading-none font-semibold', className)} {...props} />
  );
}
...
function AlertDialogCancel({ onClick, className, ...props }: AlertDialogCancelProps) {
  ...
  return (
    <Button data-slot="alert-dialog-cancel" onClick={handleClick} variant="outline" className={className} {...props} />
  );
}
